...
This linear regression model shows that there is a significant relationship between recovery rate and deviation rate since our p value is less than the alpha level of 0.05. For every integer increase in deviation rate, the chances of recovering are on average 0.02 lower. 
...
resincome <- linear_reg() %>%
  ...
  print()
```
```{r graphs_of_deviation_vs_recovery_rate_grains, message=FALSE, echo = FALSE, warning=FALSE}
food %>% 
  filter(deviation_tot<750) %>% 
...
Suggested Dietary Intake of Grains",
    ...
    y = "COVID-19 Recovery Rate")
```
According to this model, as the deviation value for grains increases by 1 unit, COVID recovery rates decrease by 0.05%. This is more stronger relationship than total deviation and suggests that grain consumption could play a more consequential role than the other food groups explored.
...
Another limitation of this study is that we did not control for vaccination rate or age. Fully vaccinated individuals have been shown to be on average 13.1 times less likely to die from COVID-19 (CDC 2020). Therefore, future studies should control for vaccination rates and age across each country and condition on them when creating other linear regression models.
