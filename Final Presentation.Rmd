...
```{r comparing_dietary_means_to_USDA}
meanfood <- food %>%
...
```
Here, we created a variable for how much the average diet in each country differs from the suggested USDA value. We did so by subtract the mean value from each food group from the USDA value and then squaring that value. We then summed up this value from each food group to create adherance total variable. We squared them because eating too much or too little of a food group should not balance each other out, rather it should increase the deviation from the suggested intake. 

To see if income level has an effect on recovery rate, we performed an ANOVA test. The results of our test indicate that 
```{r graphs_of_deviance_vs_recovery rate}
food <- food %>% 
   mutate(rec_rate=Recovered/(Confirmed)) %>% 
  mutate(adherance_grains = (30 - Grains)^2) %>% 
  mutate(adherance_veg = (40 - Vegetables2)^2) %>% 
...
food %>% 
  filter(adherance_tot<750) %>% 
ggplot(aes(x = adherance_tot, y = rec_rate)) +
...
food %>% 
  filter(adherance_tot<750) %>% 
ggplot(aes(x = adherance_tot, y = rec_rate, color=income)) +
...
res <- linear_reg() %>%
  set_engine("lm") %>%
  fit(rec_rate ~ adherance_tot, data = food) %>% 
  tidy() %>% 
  print()
```
This linear model shows that there is a significant relationship between recovery rate and deviation rate since our p value is less than the alpha level of 0.05. For every integer increase in adherance rate, the chances of recovering are on average 0.0002 lower. 
