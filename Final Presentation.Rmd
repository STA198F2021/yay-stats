...
```{r, ANOVA_obesity, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r, ANOVA_malnourish, echo=FALSE, warning=FALSE,message=FALSE}
food <- food %>% 
...
```{r graphs_of_deviance_vs_recovery rate, message=FALSE, echo = FALSE, warning=FALSE}
food %>% 
...
linear<-lm(rec_rate~deviance_tot, data=food)
...
In this model, the residuals do not seem to be even distributed around 0. Therefore, perhaps there are some predictors of the outcome that we are not including in our model. 

```{r Linear_Model_with_income, echo = FALSE, warning=FALSE, message=FALSE}

resincome <- linear_reg() %>%
  set_engine("lm") %>%
  fit(rec_rate ~ deviance_tot+income, data = food) 

reslinear<-lm(rec_rate~deviance_tot+income, data=food)
